...
router.get('/', async (req, res, next) => {
  try {
    res.json(await prisma.camp.findMany({ orderBy: { dateDebut: 'asc' } }));
  } catch (e) { next(e); }
});
...
router.put('/by-ref/:ref', validate(campSchema), async (req, res, next) => {
  try {
    res.json(await prisma.camp.update({ where: { ref: req.params.ref }, data: req.body }));
  } catch (e) { next(e); }
});

router.delete('/by-ref/:ref', async (req, res, next) => {
  try {
    await prisma.camp.delete({ where: { ref: req.params.ref } });
    res.status(204).end();
  } catch (e) { next(e); }
});
