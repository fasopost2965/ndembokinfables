...
async function main() {
  ...
  // Athletes
  await Promise.all([
    prisma.athlete.upsert({ where: { ref: 'ATH-007' }, update: {}, create: { ref: 'ATH-007', nom: 'Ndembo', prenom: 'Marcus', poste: 'Attaquant', clubActuel: 'AS Monaco', nationalite: 'Français', age: 24, valeurMarchande: 18000000, commission: 5, bio: 'Attaquant explosif formé au PSG, international espoir.', email: 'marcus@ndembo-sports.com', tel: '+33 6 12 34 56 78' } }),
    prisma.athlete.upsert({ where: { ref: 'ATH-008' }, update: {}, create: { ref: 'ATH-008', nom: 'Diallo', prenom: 'Sébastien', poste: 'Milieu central', clubActuel: 'OGC Nice', nationalite: 'Franco-Sénégalais', age: 27, valeurMarchande: 9000000, commission: 4.5, bio: 'Milieu de récupération expérimenté, capitaine à Nice.', email: 'seb.diallo@ndembo-sports.com', tel: '+33 6 98 76 54 32' } }),
  ]);

  // Devis
  const dev1 = await prisma.devis.upsert({
  ...
  // Factures
  await prisma.facture.upsert({ where: { ref: 'FAC-2026-001' }, update: {}, create: { ref: 'FAC-2026-001', clientId: clients[0].id, statut: 'Payée', date: '2026-02-01', montant: 45000 } });
  await prisma.facture.upsert({ where: { ref: 'FAC-2026-002' }, update: {}, create: { ref: 'FAC-2026-002', clientId: clients[1].id, statut: 'En retard', date: '2026-03-01', montant: 12000 } });
  ...
  // Projets
  const proj = await prisma.projet.upsert({
  ...
  // Evenements
  await prisma.evenement.upsert({ where: { ref: 'EVT-2026-001' }, update: {}, create: { ref: 'EVT-2026-001', titre: 'Gala Annuel Ndembo Sports', type: 'Gala', date: '2026-06-20', lieu: 'Monaco', budget: 15000, notes: 'Soirée de gala avec partenaires' } });
  await prisma.evenement.upsert({ where: { ref: 'EVT-2026-002' }, update: {}, create: { ref: 'EVT-2026-002', titre: 'Camp de pré-saison', type: 'Camp', date: '2026-07-05', lieu: 'Nice', budget: 8000, notes: 'Camp de préparation estival' } });

  // VIP Members
  await prisma.vipMember.upsert({ where: { ref: 'VIP-001' }, update: {}, create: { ref: 'VIP-001', clientId: clients[0].id, nom: 'Jean-Marc Doublons', tier: 'Or', email: 'jm.doublons@monaco.fr', tel: '+377 06 00 11 22', carteNFC: '4588 2201 0001 0001', dateAdhesion: '2025-01-01' } });
  await prisma.vipMember.upsert({ where: { ref: 'VIP-002' }, update: {}, create: { ref: 'VIP-002', clientId: clients[1].id, nom: 'Sophie Laurent', tier: 'Argent', email: 's.laurent@nike.fr', tel: '+33 6 55 44 33 22', carteNFC: '4588 2201 0002 0001', dateAdhesion: '2025-06-01' } });
